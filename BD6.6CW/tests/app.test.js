...
let server; 
beforeAll(async () => { 
server = http.createServer(ap); server.listen(3001); }); 
afterAll(async () => { }); 
server.close(); 
...
it("should return all employees", () => { 
  ...
  { 
  employeeId: 2, 
  name: "Priya Singh", departmentId: 2, 
  ...
  getAllEmployees.mockReturnValue (mockede Employees); 
  let result = getAllEmployees(); 
  expect(result).toEqual(mockede Employees); 
